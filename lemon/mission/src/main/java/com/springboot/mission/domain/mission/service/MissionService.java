...
import com.springboot.mission.domain.mission.repository.UserMissionRepository;
...
public class MissionService {
    ...
    public MissionResponseDTO.MyMissionPageResponse getMyMissionPage(Long userId, String token, int page, boolean status) {
        ...
        PageRequest pageRequest = PageRequest.of(page, 10);
        Page<UserMission> myMissionPage;

        // 2. status 값에 따른 리포지토리 분기 처리
        if (!status) {
            // 진행 중인 미션만 조회 (isCompleted가 false인 데이터)
            myMissionPage = userMissionRepository.findAllByUserIdAndIsCompletedFalse(userId, pageRequest);
        } else {
            // 전체 미션 조회
            myMissionPage = userMissionRepository.findAllByUserId(userId, pageRequest);
        }
